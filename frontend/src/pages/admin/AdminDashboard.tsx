...
import { Product, Order, AdminUser, UserSession } from '../../types';
import './AdminDashboard.css';

type TabType = 'dashboard' | 'customers' | 'products' | 'orders';
...
const AdminDashboard: React.FC = () => {
  ...
  const adminUser: AdminUser = JSON.parse(localStorage.getItem('adminUser') || '{}');
  const adminName: string = adminUser.name || 'Admin';

  // Get real-time customer data
  const liveCustomers: UserSession[] = getActiveSessions();
  ...
  const totalRevenue: number = orders.reduce((sum: number, order: Order) => sum + order.amount, 0);
  ...
  const handleStatusChange = (orderId: string, newStatus: Order['status']): void => {
    updateOrderStatus(orderId, newStatus);
  };

  const handleEditProduct = (product: Product): void => {
    const newPrice = prompt(`Edit price for ${product.name}:`, product.price.toString());
    if (newPrice && !isNaN(Number(newPrice))) {
      updateProduct(product.id, { price: parseInt(newPrice) });
    }
  };

  const handleViewStore = (): void => {
